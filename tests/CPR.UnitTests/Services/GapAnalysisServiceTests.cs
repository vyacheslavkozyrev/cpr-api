...
namespace CPR.UnitTests.Services
{
    public class GapAnalysisServiceTests
    {
        ...
        {
            _repoMock.Setup(r => r.GetEmployeeRecordAsync(EmployeeId, It.IsAny<CancellationToken>()))
        ...
        public async Task GetMyGapAnalysisAsync_AtHighestLevel_ReturnsNullNextPosition()
        {
            ...
            _repoMock.Setup(r => r.GetEmployeeRecordAsync(EmployeeId, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(employee);
            _repoMock.Setup(r => r.GetPositionByIdAsync(Position2Id, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(MakePosition(Position2Id, 2));
            _repoMock.Setup(r => r.GetNextPositionAsync(Position2Id, TrackId, 2, It.IsAny<CancellationToken>()))
        ...
        public async Task GetMyGapAnalysisAsync_ManagerAssessmentPresent_UsesManagerValue()
        {
            ...
            _repoMock.Setup(r => r.GetEmployeeRecordAsync(EmployeeId, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(employee);
            _repoMock.Setup(r => r.GetPositionByIdAsync(Position1Id, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(MakePosition(Position1Id, 1));
            _repoMock.Setup(r => r.GetNextPositionAsync(Position1Id, TrackId, 1, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(MakePosition(Position2Id, 2));
            _repoMock.Setup(r => r.GetPositionSkillsAsync(Position2Id, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(new List<PositionToSkill> { pts });
            _repoMock.Setup(r => r.GetEmployeeSkillsAsync(EmployeeId, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(new List<EmployeeToSkill> { empSkill });
            _repoMock.Setup(r => r.GetLinkedGoalsAsync(EmployeeId, It.IsAny<IEnumerable<Guid>>(), It.IsAny<CancellationToken>()))
        ...
        public async Task GetEmployeeGapAnalysisAsync_Director_SameDepartment_Allowed()
        {
            ...
            _repoMock.Setup(r => r.GetEmployeeRecordAsync(EmployeeId, It.IsAny<CancellationToken>()))
                     .ReturnsAsync(target);
            _repoMock.Setup(r => r.GetEmployeeRecordAsync(DirectorId, It.IsAny<CancellationToken>()))
